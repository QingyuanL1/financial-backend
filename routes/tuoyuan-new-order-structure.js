...
router.get('/:period', async (req, res) => {
    try {
        ...
        console.error('获取拓源新签订单结构与质量数据失败:', error);
        res.status(500).json({
            success: false,
            message: '获取数据失败',
            error: error.message
...
router.post('/', async (req, res) => {
    try {
        ...
        try {
            // 删除现有数据
            await connection.execute('DELETE FROM tuoyuan_new_order_structure WHERE period = ?', [period]);
            
            // 插入新数据
            for (const item of data.items) {
                await connection.execute(`
                    INSERT INTO tuoyuan_new_order_structure 
        ...
        console.error('保存拓源新签订单结构与质量数据失败:', error);
        res.status(500).json({
            success: false,
            message: '保存数据失败',
            error: error.message
