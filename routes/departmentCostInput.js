...
router.get('/:period', createBudgetMiddleware('department_cost_center_profit_loss'), async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      'SELECT * FROM department_cost_input WHERE period = ?',
      [period]
    );
    ...
    console.error('获取部门成本计入损益数据失败:', error);
...
router.post('/', async (req, res) => {
  try {
    ...
    await pool.execute(
      `INSERT INTO department_cost_input (period, data)
       VALUES (?, ?) 
    ...
    res.json({
      success: true,
      message: '部门成本计入损益数据保存成功',
      period
    });
  } catch (error) {
    console.error('保存部门成本计入损益数据失败:', error);
...
router.put('/:period', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: '部门成本计入损益数据更新成功',
      period
    });
  } catch (error) {
    console.error('更新部门成本计入损益数据失败:', error);
...
router.get('/', async (req, res) => {
  try {
    const [rows] = await pool.execute(
      'SELECT period, data, created_at, updated_at FROM department_cost_input ORDER BY period DESC'
    );
    ...
    console.error('获取所有部门成本计入损益数据失败:', error);
    res.status(500).json({ error: '获取数据失败' });
  }
});
...
router.delete('/:period', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'DELETE FROM department_cost_input WHERE period = ?',
      [period]
    );
    ...
    res.json({
      success: true,
      message: '部门成本计入损益数据删除成功',
      period
    });
  } catch (error) {
    console.error('删除部门成本计入损益数据失败:', error);
