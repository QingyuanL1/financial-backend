    ...
    try {
        ...
        const defaultData = {
            items: [
                ...
                { sequenceNo: 6, projectName: '', annualPlan: 0, currentExecutionCumulative: 0, executionProgress: 0 },
...
router.post('/', async (req, res) => {
    try {
        ...
        try {
            // 删除现有数据
            await connection.execute('DELETE FROM nanhua_major_investment WHERE period = ?', [period]);
            ...
            for (const item of data.items) {
                ...
                if (item.projectName || item.annualPlan || item.currentExecution || item.currentExecutionCumulative) {
                    await connection.execute(`
                        INSERT INTO nanhua_major_investment 
