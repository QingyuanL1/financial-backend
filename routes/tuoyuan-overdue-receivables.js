...
router.get('/:period', async (req, res) => {
    try {
        ...
        console.log(`获取拓源公司逾期应收账款数据，期间: ${period}`);
        ...
        const query = `
            SELECT 
                ...
                created_at,
                updated_at
            FROM tuoyuan_overdue_receivables 
            WHERE period = ?
            ORDER BY id ASC
        `;
        ...
        console.error('获取拓源公司逾期应收账款数据失败:', error);
        res.status(500).json({
            success: false,
            message: '获取数据失败',
            error: error.message
...
router.post('/', async (req, res) => {
    try {
        ...
        console.log(`保存拓源公司逾期应收账款数据，期间: ${period}`);
        ...
        try {
            ...
            await connection.execute(
                'DELETE FROM tuoyuan_overdue_receivables WHERE period = ?',
                [period]
            );

            // 插入新数据
            const insertQuery = `
                INSERT INTO tuoyuan_overdue_receivables 
                (period, segment_attribute, customer_attribute, year_beginning_balance, year_new_increase, current_collected, cumulative_collected, collection_progress, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            `;
        ...
        console.error('保存拓源公司逾期应收账款数据失败:', error);
        res.status(500).json({
            success: false,
            message: '保存数据失败',
            error: error.message
